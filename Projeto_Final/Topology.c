...
int Connect_To_Backup(struct Node *self, struct Node *backup)
{
	...
	if (connect(fd, res->ai_addr, res->ai_addrlen) == -1)
	{
		...
		else
		{
			...
			{
				printf("»»» Couldn't connect to backup, leaving the network \n");
				return -1;
			}
			else
			{
				...
				{
					backup->fd = fd;
					printf("»»» Connecting to backup (ID %02i)\n", backup->id);
				}
				freeaddrinfo(res);
				return 1;
			}
...
int Leaving_Neighbour(struct Node *self, struct Node *leaver, struct Neighborhood *nb, struct Expedition_Table *expt, struct Node connections[100], int *num_connections)
{
	...
	else
	{
		...
		(*num_connections)--;
		connections[(*num_connections)].fd = connections[(*num_connections)].id = -1;
		printf("    Every Node has left the newtork except me. I now am all alone. Is this the life I am meant to be living, all alone like this?\n");
		return 1;
	}
	return 1;
}
...
void Show_Topology(struct Neighborhood *nb)
{
	...
	printf("EXTERNAL NEIGHBOUR \n");
	if (nb->external.id != -1)
		printf("id: %i \t ip: %s \t port: %s \n", nb->external.id, nb->external.ip, nb->external.port);
	printf("\n");

	printf("BACKUP NEIGHBOUR \n");
	if (nb->backup.id != -1)
