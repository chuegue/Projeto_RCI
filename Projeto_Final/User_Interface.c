		...
		if (connect(fd, res->ai_addr, res->ai_addrlen) == -1)
		{
			...
			else
			{
				...
				else
				{
					...
					printf("»»» Connecting to the Node with ID %02i\n", commands->bootid);
...
void join(struct User_Commands *commands, struct Node *self, struct Node *other, struct Neighborhood *nb, struct Expedition_Table *expt, char *nodesip, char *nodesport)
{
	...
	{
		printf("»»» Message received from the Nodes server isn't correct. Didn't join.\n");
		return;
	}
	if ((tok = strtok(NULL, "\n")) == NULL) // primeiro nó
	{
		printf("»»» I am the first node in the network\n");
	...
	else // segundo nó
	{
		...
		if (is_repeated)
		{
			...
			printf("»»» ID inserted already in use, from now on this Node has ID %02i\n", t);
		}
		...
		if (is_repeated)
			commands->id = t;
		printf("»»» I am not the first node in the network. ");
		djoin(commands, self, other, nb, expt);
	}
	free(received);
	if (self->net != -1)
	{
		...
		printf("»»» I registered on the Nodes server\n");
		received = transrecieveUDP(nodesip, nodesport, buffer, strlen(buffer), &n_received);
		if (received == (char *)NULL)
			return;
		...
		{
			free(received);
			printf("    Message received from the Nodes server while registering isn't correct.\nNot sure if registration was sucessful.\n");
			return;
		}
		printf("    I received the appropriate response from the Nodes server\n");
		free(received);
	}
}
...
void leave(struct Node *self, struct Neighborhood *nb, struct Expedition_Table *expt, char *nodesip, char *nodesport, int social)
{
	if (social)
	{
		...
		printf("»»» I unregistered on the Nodes server\n");
		...
		{
			free(received);
			printf("    Message received from the Nodes server while unregistering isn't correct.\nNot sure if unregistration was sucessful.\n");
			return;
		}
		printf("    I received the appropriate response from the Nodes server\n");
		free(received);
